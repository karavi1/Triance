...
def test_update_exercise(db):
    created = crud_exercise.create_exercise(db, ExerciseCreate(name="Row", primary_muscles=["back"], category=ExerciseGroup.PULL), None)
    updated = crud_exercise.update_exercise(db, created.id, ExerciseUpdate(name="Barbell Row", primary_muscles=["back", "biceps"]), None)
    assert updated.name == "Barbell Row"
    assert "biceps" in updated.primary_muscles


def test_update_exercise_invalid_id(db):
    result = crud_exercise.update_exercise(db, uuid4(), ExerciseUpdate(name="X"), None)
...
def test_delete_exercise(db):
    created = crud_exercise.create_exercise(db, ExerciseCreate(name="Press", primary_muscles=["s"], category=ExerciseGroup.PUSH), None)
    res = crud_exercise.delete_exercise(db, created.id, None)
    assert res is True
    assert crud_exercise.get_exercise_by_id(db, created.id, None) is None


def test_delete_exercise_invalid_id(db):
    assert not crud_exercise.delete_exercise(db, uuid4(), None)
